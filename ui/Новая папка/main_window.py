...
# Імпортуємо наш кастомний заголовок
from .custom_title_bar import CustomTitleBar 

# Імпортуємо бібліотеки для моніторингу
import psutil
try:
    import pynvml
...
# Виправлено синтаксис try-except
try:
...
def _icon_voikan_logo(size=32):
    def _paint(p: QPainter, s: int):
        ...
        p.drawText(QRectF(0, 0, s, s), Qt.AlignCenter, "V")
...
class MainWindow(QMainWindow):
    def __init__(self, app):
        ...
        menu_items = [
            ("Аудіо Suno", "", self.page_audio),
            ("Фото Vertex", "", self.page_photo),
            ("Монтаж YouTube", "", self.page_videos),
            ("YT Планер", "", self.page_planner),
        ...
        if hasattr(self.title_bar, "update_button"):
            self.title_bar.update_button.clicked.connect(self._on_check_for_updates_clicked)
    ...
    def _make_control_panel(self) -> QWidget:
        ...
        v_lay.setSpacing(12)
        gb1 = QGroupBox("Керування")
        v1 = QVBoxLayout(gb1)
        self.monitor_widget = SystemMonitorWidget()
        v1.addWidget(self.monitor_widget)
        self.btn_start = AnimatedPushButton("старт")
        self.btn_start.setObjectName("startBtn")
        self.btn_stop = AnimatedPushButton("стоп")
        self.btn_stop.setObjectName("stopBtn")
        self.btn_stop.setEnabled(False)
        self.btn_start.clicked.connect(self._on_start_clicked)
        self.btn_stop.clicked.connect(self._on_stop_clicked)
        v1.addWidget(self.btn_start)
        v1.addWidget(self.btn_stop)
        ...
        self.btn_clear_logs.setObjectName("clearLogsBtn") # Додано ім'я
        self.btn_clear_logs.clicked.connect(self.logs.clear)
        v2.addWidget(self.logs, 1)
        v2.addWidget(self.btn_clear_logs)
    ...
    def _apply_initial_geometry(self):
        ...
        if WINDOW_MIN_W or WINDOW_MIN_H:
            self.setMinimumSize(int(WINDOW_MIN_W * self.dpi_scale), int(WINDOW_MIN_H * self.dpi_scale))
        s = QSettings(ORG_NAME, APP_NAME)
        w = s.value("win/width", type=int) if REMEMBER_LAST_SIZE else None
        h = s.value("win/height", type=int) if REMEMBER_LAST_SIZE else None
    ...
    def _apply_scale(self, scale: float):
        ...
        for i in range(self.menu.count()):
            self.menu.item(i).setSizeHint(QSize(220, max(56, int(68 * scale))))
    ...
    def _sync_right_panel_for(self, page):
        ...
        val = self._progress_val[page]
        lbl = self._progress_lbl[page]
        self.progress.setValue(val)
        self.progress.setFormat(f"{lbl} — {val}%" if lbl else "%p%")
        
    def _on_start_clicked(self): 
        page = self._current_page()
        if hasattr(page, "handle_start"):
            page.handle_start(True) # Завжди запускаємо в "авто"
