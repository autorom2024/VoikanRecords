...
import os, time, signal, queue, threading, subprocess, json, random
...
def _motion_chain(mv: Dict, W:int,H:int,fps:int) -> Tuple[str,str]:
    ...
    ### === ВИПРАВЛЕНО: Бекенд сам перетворює текст в код ===
    direction_map = {
        "ліво→право": "lr", "право→ліво": "rl", "вверх": "up", "вниз": "down",
        "зум in": "zin", "зум out": "zout", "обертання": "rotate", "тряска": "shake"
    }
    direction_text = str(mv.get("direction","lr")).lower().replace(" ", "")
    direction = direction_map.get(direction_text, "lr")
...
def _eq_chain(eq: Dict, W:int, H:int) -> Tuple[str,int,int,bool,str,str]:
    ### === ВИПРАВЛЕНО: Якщо еквалайзер вимкнено, нічого не робити ===
    if not eq or not eq.get("enabled", False):
        return "", 0, 0, False, "", ""
...
def _ffmpeg_cmd_single(cfg: Dict, song: Path, bg_img: Optional[Path], out_path: Path,
    ...
    threads=_safe_int(cfg.get("threads",0),0)
    ...
    ### === ФІНАЛЬНИЙ МАЙСТЕР-ПЕРЕМИКАЧ ===
    motion_enabled = cfg.get("motion_ui", {}).get("enabled", False)
    eq_enabled = cfg.get("eq_ui", {}).get("enabled", False)
    
    if (motion_enabled or eq_enabled) and use_cuda_fx:
        use_cuda_fx = False
        try:
            reason = "Motion FX" if motion_enabled and eq_enabled is False else "Equalizer" if eq_enabled else "Motion FX and Equalizer"
            status_q.put({"type": "log", "msg": f"[CFG] CUDA filters disabled due to active {reason} (using stable CPU path)"})
        except Exception:
            pass
    ...
    if use_cuda_fx:
        ...
        for i,e in enumerate(effs):
            ...
            fparts.append(f"{last}[fx{idx}]overlay_cuda=x=0:y=0[v_{e.nick}]")
            last = f"[v_{e.nick}]"
        ...
        last = bg_after
        if eq_txt:
            fparts.append(f"{last}{tag_up}overlay=x=0:y={y_top}:shortest=1[v_eq_up]")
            last = "[v_eq_up]"
            if mirror and tag_dn:
                fparts.append(f"{last}{tag_dn}overlay=x=0:y={y_top+eq_h}:shortest=1[v_eq_dn]")
                last = "[v_eq_dn]"
        
        base_idx = 2 if has_bg else 1
        for i,e in enumerate(effs):
            idx = base_idx + i
            fparts.append(f"{last}[{idx}:v]overlay=x=0:y=0:shortest=1[v_{e.nick}]")
            last = f"[v_{e.nick}]"
    ...
    if album_enabled and album_sec > 0:
        cmd += ["-t", str(album_sec)]
    if threads>0:
    ...
    while not _STOP_ALL:
        ...
        try:
            ...
            if job.album_index > 0:
                ...
                status_q.put({"type":"log","msg":f"▶ {worker_tag} Рендер: {job.song.name}"})
            ...
            if code==0:
                if job.album_index > 0:
                    ...
                    status_q.put({"type":"log","msg":f"✅ {worker_tag} Готово: {out_path.name}"})
...
def _build_album_audio(tracks: List[Path], album_sec: int, status_q: "queue.Queue[dict]", album_idx: int) -> Tuple[Path, float]:
    ...
    out_wav = CACHE_DIR / f"album_audio_{album_idx}_{int(time.time() * 1000)}.wav"
...
def start_video_jobs(cfg: Dict, status_q: "queue.Queue[dict]", cancel_event: threading.Event):
    ...
    want=max(1,min(1000,_safe_int(cfg.get("songs",1),1)))
    ...
    def _manager():
        ...
        try:
            ...
            if album_enabled:
                idx = 1
                while True:
                    if _STOP_ALL or cancel_event.is_set(): break
                    batch, current_position = _select_batch(songs_all, current_position, want, until_material)
                    if not batch: break
                    
                    status_q.put({"type":"log","msg":f"[PLAN] Альбом {idx}: {len(batch)} трек(ів), ціль {album_sec or '—'}s"})
                    album_wav, dur = _build_album_audio(batch, album_sec, status_q, idx)
                    
                    if album_wav and dur > 0:
                        cfg_local = dict(cfg); cfg_local["album_combined"] = True
                        qin.put(RenderJob(cfg_local, album_wav, bg_pool.next(), out_path, idx))
                    else:
                        status_q.put({"type":"log","msg":f"⚠ Пропуск альбому {idx}: не вдалося створити аудіо."})

                    idx += 1
                    if not until_material: break
            else:
                while True:
                    if _STOP_ALL or cancel_event.is_set(): break
                    batch, current_position = _select_batch(songs_all, current_position, want, until_material)
            ...
            _save_processing_state({
                ...
                "processed_songs": [str(p) for p in songs_all[:current_position]]
            })
            status_q.put({"type":"log","msg":f"[SAVE] Позиція збережена: {current_position}/{len(songs_all)}"})
