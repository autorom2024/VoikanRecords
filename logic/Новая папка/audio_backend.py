...
- build_albums_pipeline(...): формує альбоми з чистими логами (лише підсумкові
  перейменовані треки з ✅ та назви альбомів). Сайдбар для альбомів — не частіше 5с.
...
WAIT_MAX_SEC   = int(os.environ.get("KIE_WAIT_MAX_SEC", 500))   # максимум очікування задачі
POLL_STEP_SEC  = int(os.environ.get("KIE_POLL_STEP_SEC", 5))    # тики/полінг КОЖНІ 5с
POLL_ERR_EVERY = int(os.environ.get("KIE_POLL_ERR_EVERY", 60))  # лог 4xx/5xx не частіше хвилини

# тики очікування в логи (за замовчуванням вимкнено, щоб не «засирати чат»)
    ...
    while True:
        ...
        try:
            ...
            if r.status_code >= 400:
                now = int(time.time())
                if now - last_err_log >= POLL_ERR_EVERY:
                    last_err_log = now
...
def run_suno_pipeline(
    *,
    ...
    # назви (user → GPT → style → дефолт), гарантуємо унікальність
    titles: List[str] = []
    if user_titles:
    ...
    made = 0
    last_progress = 0
    rounds = int(batches)  # 1 POST -> 2 треки
    ...
    for ri in range(rounds):
        ...
        # межі прогресу: очікування займає ~80% частки цього раунду
        ...
        def on_wait(elapsed: int, max_wait: int):
            """Оновлюємо лише сайдбар; у лог НЕ пишемо тики щоб не засмічувати."""
            ...
            if cur_overall > last_progress:
                ...
                if LOG_WAIT_TICKS:
                    _log(status_queue, label)  # опційно, через env, за замовчуванням вимкнено

        # poll з тиками кожні 5с
        try:
        ...
        # беремо рівно 2 треки з цього POST
        for it in items[:2]:
            if _cancelled(cancel_event):
                _log(status_queue, "🛑 Зупинено."); return 1
            urls = _pick_urls(it)
...
def build_albums_pipeline(
    *,
    ...
    """
    Чисті логи альбомів:
      • На початку — назва альбому.
      • Для кожного треку — ТІЛЬКИ: ✅ [{i}/{n}] <кінцеве ім'я файлу>.
      • Наприкінці — 🏁 Готово — <альбом>.
    Назви альбомів/треків унікальні, на основі style_prompt (якщо не задано генератором).
    Сайдбар оновлюється не частіше 5с.
    """
...
__all__ = ["run_suno_pipeline", "build_albums_pipeline"]
